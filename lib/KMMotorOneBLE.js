    ...
    static startScanToCreateInstance(time = 2000,allowDuplicates=false){
        ...
        if(noble.state === 'poweredOn'){
            ...
            _startScanTimeoutID=setTimeout(()=>{
                ...
                /**
                 * BLEスキャナーのタイムアウト時
                 * @event KMMotorOneBLE#scanTimeout
                 * @type {KMMotorOneBLE}
                 * @example
                 *
    ...
    static _discoverLis(nobleperipheral){
        ...
        if(!_motorsByUUID[nobleperipheral.uuid]){
           let motor= new KMMotorOneBLE(nobleperipheral);//info::検出したモーターは全てインスタンス化
            ...
            /**
             * 新規モーターの発見時
             * @event KMMotorOneBLE#discoverNewMotor
             * @type {KMMotorOneBLE}
             * @property {object} KMMotorOneBLE {@link KMMotorOneBLE}
             * @example
             *
        ...
        if(_motorsByUUID[nobleperipheral.uuid]){
            ...
            /**
             * モーターの発見時
             * @event KMMotorOneBLE#discoverMotor
             * @type {KMMotorOneBLE}
             * @property {object} KMMotorOneBLE {@link KMMotorOneBLE}
             * @example
             *
             * KMMotorOneBLE.on(KMMotorOneBLE.EVENT_TYPE.discoverMotor,function(kMMotorOne){
             *      console.log('discoverMotor:'+kMMotorOne.deviceInfo.name);
             * });
             */
        }
