    ...
    */
    ...
    constructor(connect_type,kmcom){
        ...
        this._MOTOR_COMMAND={
            ...
            /** ダイレクトティーチングの記録名設定 */ setMotionName:0xAF,

            /** ダイレクトティーチング内容把握 */ readMotionInfo:0xB0,
            /** ダイレクトティーチング使用状況把握 */ readMotionUsage:0xB1,
            ...
            /** 内部シリアル番号 */ readDeviceSerial:0xFA
        };
         ...
         * @private
         */
        this._KMCom._onMotorLogCB=(cmdID, logObject)=>{
            let cmdName=this._REV_MOTOR_COMMAND[cmdID]?this._REV_MOTOR_COMMAND[cmdID]:cmdID;
            if(logObject){
                logObject['cmdName']=cmdName;
            }
            this.emit(this.EVENT_TYPE.motorLog,logObject);
        };
    }
    /********************************************
     * プロパティ
     ...
     */
    cmdStopDoingTaskset(){
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.stopDoingTaskset);
    }
    ...
    cmdStartPlaybackMotion(index = 0,repeating = 0,start_position = 0){
        ...
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.startPlaybackMotion,buffer);
    }
     ...
     * @param {number} repeating int 繰り返し回数 0は無制限
     * @param {KMMotorCommandKMOne.cmdPreparePlaybackMotion_START_POSITION} start_position int スタート位置の設定<br>
     * 0:START_POSITION_ABS:記憶された開始位置（絶対座標）からスタート<br>
     * 1:START_POSITION_CURRENT:現在の位置を開始位置としてスタート
     *
     * @desc KM-1 は index: 0~9 まで。（10個のメモリバンク。）それぞれ 約2分記録可能。<br>
     * ティーチング（動作の記録）は、コマンド（ティーチング）を参照<br>https://document.keigan-motor.com/motor-control-command/teaching.html<br>
     */
    ...
    cmdSetTasksetName(name){
        ...
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.setTasksetName,buffer);
    }
     ...
     * @desc ダイレクトティーチングによるモーションの記録を開始する
    ...
    cmdStartTeachingMotion(index = 0,lengthRecordingTime = 0){
        ...
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.startTeachingMotion,buffer);
    }
     ...
     *
     */
    cmdPrepareTeachingMotion(index = 0,lengthRecordingTime = 0){
        let buffer = new ArrayBuffer(6);
        new DataView(buffer).setUint16(0,Math.abs(parseInt(index,10)));
        new DataView(buffer).setUint32(2,Math.abs(parseInt(lengthRecordingTime,10)));
     ...
     */
    cmdStartTeachingMotionFromPrep(){
    ...
    /**
     * @summary ダイレクトティーチングの記録名設定
     * @desc モーションの記録名を設定する。（これから記録するものに対して）
     */
    cmdSetMotionName(name){
        let buffer= (new Uint8Array([].map.call(name, function(c) {
            return c.charCodeAt(0);
        }))).buffer;
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.setMotionName,buffer);
    }
    ...
    //---------------------//
    /**
     * @summary IMU(ジャイロ)の値取得(通知)を開始する  [Deprecated::2019/01]
     * @desc cmdStartIMUMeasurementに改名
     * @ignore
     */
    cmdEnableIMU(){
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.enableIMU);
        console.log("cmdEnableIMU has been deprecated and changed to cmdStartIMUMeasurement");
    }
     ...
     */
    cmdStartIMUMeasurement(){
        this.cmdEnableIMU();
    }


    /**
     * @summary IMU(ジャイロ)の値取得(通知)を停止する [Deprecated::2019/01]
     * @desc stopIMUMeasurementIMUに改名
     * @ignore
     */
    cmdDisableIMU(){
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.disableIMU);
        console.log("cmdDisableIMU has been deprecated and changed to cmdStopIMUMeasurementIMU");
    }

    /**
     * @summary IMU(ジャイロ)の値取得(通知)を停止する
     */
    cmdStopIMUMeasurementIMU(){
        this.cmdDisableIMU();
    }
     ...
     * @ignore
     */
    cmdStartMotorMeasurement(){
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.enableMotorMeasurement);
    }
    /**
     * @summary モーターの測定値（位置・速度・トルク）出力を停止する
     * @ignore
     */
    cmdStopMotorMeasurement(){
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.disableMotorMeasurement);
    }
    ...
    cmdEnableCheckSum(isEnabled){
        ...
        this._KMCom._sendMotorCommand('MOTOR_CONTROL',this._MOTOR_COMMAND.enableCheckSum,buffer);
    }
    ...
    cmdTeachingInterval(interval){
        ...
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.teachingInterval,buffer);
    }
    /**
     * @summary 記憶再生時の再生間隔
     * @desc  記憶再生時の再生間隔
    ...
    cmdPlaybackInterval(interval){
        ...
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.playbackInterval,buffer);
    }
    ...
    cmdMotorMeasurementInterval(cmdMotorMeasurementInterval_MOTOR_MEAS_INTERVAL=4){
        ...
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.motorMeasurementInterval,buffer);
    }
    ...
    cmdMotorMeasurementByDefault(isEnabled){
        let buffer = new ArrayBuffer(1);
        new DataView(buffer).setUint8(0,parseInt(isEnabled,10));
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.motorMeasurementByDefault,buffer);
    }
    ...
    cmdInterface(bitFlg){
        ...
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.interface,buffer);
    }
    ...
    cmdIMUMeasurementInterval(cmdIMUMeasurementInterval_IMU_MEAS_INTERVAL=4){
        ...
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.iMUMeasurementInterval,buffer);
    }
    ...
    cmdIMUMeasurementByDefault(isEnabled){
        let buffer = new ArrayBuffer(1);
        new DataView(buffer).setUint8(0,parseInt(isEnabled,10));
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.iMUMeasurementByDefault,buffer);
    }
    ...
    cmdReadDeviceInfo(){
        ...
        // let self=this;
        // let rd=self._MOTOR_COMMAND.readDeviceInfo;
        // return new Promise((lastresolve, lastreject)=> {
        //     new Promise((resolve, reject)=> {
        //         let ccp=new _KMNotifyPromis(rd,this._REV_MOTOR_COMMAND[rd],this._notifyPromisList,resolve,reject,1000);//notify経由のresultをPromisと紐付け
        //         let buffer = new ArrayBuffer(1);
        //         new DataView(buffer).setUint8(0, parseInt(dataType, 10));
        //         self._KMCom._sendMotorCommand('MOTOR_SETTING',self._MOTOR_COMMAND.readDeviceInfo, buffer,ccp);
        //     }).then((res)=>{
        //         lastresolve(res[Object.keys(res)[0]]);
        //     }).catch((msg)=>{
        //         lastreject(msg);
    ...
    cmdSetI2CSlaveAddress(address){
        ...
        this._KMCom._sendMotorCommand('MOTOR_SETTING',this._MOTOR_COMMAND.setI2CSlaveAddress,buffer);
    }
