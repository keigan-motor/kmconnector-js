...
class KMComBLE extends KMComBase{
    ...
    static get MOTOR_BLE_CRS(){
        return {
            'MOTOR_CONTROL':'f140000189364d35a0eddfcd795baa8c',//::info::新
            //'MOTOR_LED':'f140000389364d35a0eddfcd795baa8c',//モーターのLEDを取り扱う info:: MOTOR_CONTROL::bleLedで代用
            'MOTOR_MEASUREMENT':'f140000489364d35a0eddfcd795baa8c',//::info::新
            'MOTOR_IMU_MEASUREMENT':'f140000589364d35a0eddfcd795baa8c',//::info::新
           // 'MOTOR_SETTING':'f140000689364d35a0eddfcd795baa8c',
            'MOTOR_SETTING':'f140000189364d35a0eddfcd795baa8c',//::info::新
            'MOTOR_LOG':'f140000289364d35a0eddfcd795baa8c'//::info::新
        };
    }
                            ...
                            if (service['uuid'] === this.constructor.MOTOR_BLE_SERVICE_UUID) {
                                ...
                                service.discoverCharacteristics([],(err,characteristics) =>{
                                    ...
                                    new Promise((resolve, reject) => {
                                        ...
                                        return new Promise((resolve, reject) => {
                                            if (this._characteristics['MOTOR_LOG']) {
                                                this._characteristics['MOTOR_LOG'].removeAllListeners('data');
                                                this._characteristics['MOTOR_LOG'].on('data', this._onBleMotorLog.bind(this));
                                                this._characteristics['MOTOR_LOG'].subscribe((error) => {
                                                    if (error) {
                                                        let errmsg = 'err:MOTOR_SETTING subscribe -> :' + error;
                                                        console.log(errmsg);
                                                        reject(errmsg);
    ...
    _onBleMotorSetting(data){
        if(! data instanceof Buffer){return;}
        let uint8Array=new Uint8Array(data);//info::一旦コピーする必要がある
        let dv=new DataView(uint8Array.buffer);//5+nバイト　データ仕様　https://docs.google.com/spreadsheets/d/1uxJu86Xe8KbIlxu5oPFv9KQdvHY33-NIy0cdSgInoUk/edit#gid=1000482383
